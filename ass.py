...
# ここに文言を追加します
st.markdown("""
本ツールは、エスティマンドを成立させる試験運用条件をプロトコルから抽出します。
サイドバーにエスティマンドを記載し、プロトコルを読み取り解析ボタンを押してください。

さらに「CTQを深堀りするボタン」を押すことで、「試験解釈を維持するCTQ状態」をCTTIの原則を参照に出力します。
本ツールではCTQ要因は個別データや手順ではなく、
エスティマンド解釈を成立させる運用状態として定義しています。
また同時に出力される、観測データはCTQの状態が実際に維持されているかを確認するための情報と定義しています。
""")
...
### 3.モニタリング項目
-### 1. エスティマンド解析と### 2. 有害事象（AE）発生時の対応規定で得られた結果をもとに下記を記述せよ。
1. 対象集団、変数、中間事象、有害事象の「定義」に関する規定をそれぞれ記述
2. 対象集団、変数、中間事象、有害事象の定義を成立させるために直接必要な守るべき条件をそれぞれ記述
3. 上記規定が守られているかどうか観測するために必要なデータをそれぞれ記述

### 【基本原則】
- 各要素の見出し（例：## ii. 対象集団）の直後に詳細解析を記述。
- 末尾には「【該当文章の直接引用】」を設け、原文を抜粋すること。
- 「--- [PAGE X] ---」をページ数の根拠とすること。
- 推測・補完を禁止し、不明な場合は「不明」と明記する。記載がない場合は「該当なし」とする。
---
{text_data[:80000]}
"""
...
直接導ける内容のみ記述すること。
一般的なGCP要求事項、抽象的な品質概念をCTQとして出力してはならない。
...
【Safety/GCP運用CTQ】
エスティマンドとは別にSafety/GCP運用CTQを独立して記述すること。
### 2. 有害事象（AE）発生時の対応規定
- 治験薬の減量（Dose Reduction）や休薬（Interruption）の基準
- AE/SAEの報告手順・報告期限
- 投与中止（Discontinuation）に至るAEの基準
